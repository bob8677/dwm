...
static void setgaps(int oh, int ih, int iv);
...
deck(Monitor *m) {
        ...
        mw = m->nmaster >= n ? m->ww - 2*m->gappov*oe : (m->nmaster ? (m->ww + m->gappiv*ie - 2*m->gappov*oe)* m->mfact : 0);
	...
	if(n > m->nmaster) {
        ...
        for (i = 0, my = ty = m->gappoh*oe, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
                if (i < m->nmaster) {
                        ...
                        resize(c, m->wx + m->gappov*oe, m->wy + my, mw - (2*c->bw) - (m->nmaster < n ?  m->gappiv*ie : 0), h - (2*c->bw), 0);
                        if (my + HEIGHT(c) < m->wh)
                                my += HEIGHT(c) + m->gappih*ie;
                } else {
                        r = n - i;
                        h = m->wh - 2*m->gappoh*oe + 100/(n - m->nmaster) - 100;
...
monocle(Monitor *m)
{
	unsigned int n = 0, e = enablegaps*(1 - smartgaps);
	Client *c;
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
		resize(c, m->wx + m->gappov*e, m->wy + m->gappoh*e, m->ww - (2 * c->bw) - 2*m->gappov*e, m->wh - (2 * c->bw) - 2*m->gappoh*e, 0);
	}
}
...
setgaps(int oh, int ih, int iv)
{
	...
	selmon->gappoh = oh;
	selmon->gappov = oh*selmon->ww/selmon->wh;
	selmon->gappih = ih;
	selmon->gappiv = iv;
	arrange(selmon);
}
